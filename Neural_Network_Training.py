...
lr_init   = 0.01  
lr1 = (lr_init/np.sqrt(num_input))*np.sqrt(num_input/hiddenlayer1)
lr2 = (lr_init/np.sqrt(num_input))*np.sqrt(hiddenlayer1/hiddenlayer2)
lr3 = (lr_init/np.sqrt(num_input))*np.sqrt(hiddenlayer2/num_output)
epoch     = 6
minibatch_size = 10
...
class Neural(nn.Module):
    def __init__(self):
        ...
        #torch.nn.init.normal(self.fc1.weight,mean=mean_init,std=std_init)
        #torch.nn.init.normal(self.fc2.weight,mean=mean_init,std=std_init)
        #torch.nn.init.normal(self.fc3.weight,mean=mean_init,std=std_init)
        
        #torch.nn.init.normal(self.fc1.bias,mean=mean_init,std=std_init)
        #torch.nn.init.normal(self.fc2.bias,mean=mean_init,std=std_init)
        #torch.nn.init.normal(self.fc3.bias,mean=mean_init,std=std_init)
...
optimizer = torch.optim.SGD([
    {'params':neu.fc1.parameters(),'lr':lr1},
    {'params':neu.fc2.parameters(),'lr':lr2},
    {'params':neu.fc3.parameters(),'lr':lr3}
])
